...
public class MultipleAStarSearch extends Search {
		...
		while(!goals.isEmpty()) {
			...
			if(!visited.contains(state.getCell())) {
				...
				if(goals.contains(state.getCell())) {
					...
					PriorityQueue<AStarState> tmp = new PriorityQueue<AStarState>(10, new Comparator<AStarState>() {
						@Override
						public int compare(AStarState n1, AStarState n2) {
							if(n1.getHeuristic() > n2.getHeuristic()) {
								return +1;
					...
					while(!pq.isEmpty()) {
						...
						AStarState b = pq.remove();
						b.setHeuristic(heuristic(b.getCell().getX(), b.getCell().getY(), b.getPathLength()));
						tmp.add(b);
					}
					pq = tmp;
	...
	private double heuristic(int x, int y, int pathLength) {
		...
		for(Cell q : goals) {
			if(distance(x, y, q.getX(), q.getY()) < distance) {
				distance = distance(x, y, q.getX(), q.getY());
				target = q;
			}
